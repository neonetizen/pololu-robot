...
void Odometry::update_odom(int left_encoder_counts, int right_encoder_counts, float &x, float &y, float &theta){
  ...
  if(_deadreckoning) {
    ...
    _theta = angleRate * 0.001;
  } else {
    _theta += (right_encoder_counts - left_encoder_counts)/_w;
  }
  ...
  _x = (right_encoder_counts + left_encoder_counts) / 2 * cos(_theta);
  // CALCULATE _y BASED ON THE FORMULA FROM THE LECTURES
  _y = (right_encoder_counts + left_encoder_counts) / 2 * sin(_theta);
